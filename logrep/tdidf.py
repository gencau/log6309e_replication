...
def Text_TFIDF_Generator(x_train, x_test, texttype):
    ...
    for i in range(0, int(n_bat)+1):
        ...
        new_list.append({k: x_train[k] for k in list(
            x_train.keys())[split_start:split_end]})
    ...
    for i in range(0, int(n_bat)+1):
        split_start = i*batch_size
        split_end = (i+1)*batch_size
        new_list.append({k: x_test[k] for k in list(
            x_test.keys())[split_start:split_end]})
...
save_path = os.path.join(data_dir, 'TEXT_TFIDF_' +
                         ttype + '_' + log_name + '.npz')
